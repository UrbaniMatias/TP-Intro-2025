export default class Usuario {
  constructor(id, nombre, email, fecha_registro, fecha_de_nacimiento) {
    ...
    if (typeof fecha_registro !== "string")
      throw new Error("fecha_registro inválida");

    if (typeof fecha_de_nacimiento !== "string")
      throw new Error("fecha_de_nacimiento inválida");

    this.id = id;
    this.nombre = nombre;
    this.email = email;
    this.fecha_registro = (new Date(fecha_registro)).toLocaleLowerCase();
    this.fecha_de_nacimiento = (new Date(fecha_de_nacimiento)).toLocaleLowerCase();
  }
}
